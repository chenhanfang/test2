...
def get_event_list(request):###查询发布会接口
    eid = request.GET.get('eid','')
    name = request.GET.get('name','')
...
# def add_guest(request):####添加嘉宾接口
#     eid = request.GET.get('eid','')
#     realname = request.GET.get('realname','')
#     phone = request.GET.get('phone','')
#     email = request.GET.get('email','')
#     if eid == '' or realname == '' or phone == '':
#         return JsonResponse({'status':10021,'message':'parametr error'})
#     result = Event.objects.filter(id=eid)
#     if not result:
...
#     try:
#         Guest.objects.create(realname=realname,phone=int(phone),
#                              email=email,sign=0,event_id=int(eid))
...
def add_guest(request):
    ...
    realname = request.GET.get('realname','')       # 姓名
    phone = request.GET.get('phone','')             # 手机号
    email = request.GET.get('email','')             # 邮箱
    gid = request.GET.get('gid','')
    if eid =='' or realname == '' or phone == '' or gid == '':
        return JsonResponse({'status':10021,'message':'parameter error'})

    result = Event.objects.filter(event_id=eid)
    if not result:
        return JsonResponse({'status':10022,'message':'event id null'})
    result = Event.objects.filter(id = gid)
    if result:
        return JsonResponse({'status':10029,'message':'guest id already exists'})
...
def search_guest(request):####查询嘉宾接口
    eid = request.GET.get('eid','')
    phone = request.GET.get('phone','')
    if eid == '':
